...
public class DTOModelMapperTest extends ErrorCollectorRule {
    ...
    public void testMapMovieToMovieResponse() {
        Movie movie = new Movie(1L, "The Shawshank Redemption", new Date(1994), new Time(2), 9.3F,
                "This is The storyline", "poster.png", "R", new ArrayList<>(),
                new ArrayList<>(), new ArrayList<>(), new ArrayList<>());
        MovieResponse movieResponse = DTOModelMapper.mapMovieToMovieResponse(movie);
        this.collector.checkThat("The movie id should be 1", movieResponse.getId(), is(1L));
        this.collector.checkThat("Mismatch movie name", movieResponse.getName(), is(equalTo("The Shawshank Redemption")));
        this.collector.checkThat("The movie rating should be 9.3", movieResponse.getRating(), is(9.3F));
        this.collector.checkThat("Mismatch movie storyline", movieResponse.getStoryline(), is(equalTo("This is The storyline")));
        this.collector.checkThat("Mismatch movie poster", movieResponse.getPoster(), is(equalTo("poster.png")));
        this.collector.checkThat("The movie Rated should be 'R'", movieResponse.getRated(), is(equalTo("R")));
    }
}
